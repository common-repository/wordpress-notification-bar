...
class SeedProd_WordPress_Notification_Bar{
    ...
    function render_notification_bar($hook){
        ...
        if($hook == 'settings_page_seed_wnb' || ! is_admin() && isset($options['enabled'])){
            ...
            $data = array( 
				'msg' => sanitize_text_field($options['msg']),
				'button_link' => sanitize_url($options['button_link']),
				'button_label' => sanitize_key($options['button_label']),
				'button_target' => sanitize_text_field($button_target),
                );
